    ...
    return `
        <div class="battery-option ${isPrimary ? 'is-primary' : ''}">
          ...
          <div class="battery-option__grid">
            ...
            <div><span>Повна енергія комплекту</span><strong>${formatEnergyWh(config.totalStoredWh)}</strong></div>
            ...
            <div><span>Чи вистачає на бажаний час роботи</span><strong>${formatNumber(config.targetCoverageRatio * 100, 0)}%</strong></div>
            <div><span>Чи вистачає запасу</span><strong>${formatNumber(config.reserveCoverageRatio * 100, 0)}%</strong></div>
          </div>
        </div>
      `;
  }
