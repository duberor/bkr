...
class ConsumerModal extends BaseElement {
      ...
      <div class="consumer-modal__backdrop">
        <div class="consumer-modal__dialog" role="dialog" aria-modal="true" aria-label="${heading}">
          <div class="consumer-modal__head">
            ...
            <div class="consumer-modal__head-actions">
              <button class="consumer-modal__star" type="button" title="Зберегти в типові прилади" aria-label="Зберегти в типові прилади">★</button>
  ...
  afterRender() {
    ...
    this.shadowRoot
      .querySelector('.consumer-modal__star')
      ?.addEventListener('click', this.handleSaveToLibrary);
    this.shadowRoot
  ...
  handleSaveToLibrary = () => {
    const nameErr = this.validateField('name');
    const powerErr = this.validateField('power');
    if (nameErr || powerErr) {
      this.errors = { ...this.errors, name: nameErr, power: powerErr };
      this.dispatchEvent(
        new CustomEvent('consumer-modal-invalid', {
          detail: { errors: [nameErr || powerErr] },
          bubbles: true,
          composed: true,
        }),
      );
    ...
    const template = {
      ...
      power: parseLocaleNumber(this.formData.power) || 0,
      hoursPerDay: parseLocaleNumber(this.formData.hoursPerDay) || 0,
      surgePower: parseLocaleNumber(this.formData.surgePower) || 0,
      priority: this.formData.priority || 'medium',
      usageProfile: this.formData.usageProfile || 'day',
      notes: String(this.formData.notes || '').trim(),
    };
    this.dispatchEvent(
      new CustomEvent('consumer-save-to-library', {
        detail: { template },
        bubbles: true,
        composed: true,
      }),
