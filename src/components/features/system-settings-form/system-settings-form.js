...
class SystemSettingsForm extends BaseElement {
  constructor() {
    ...
    this.errors = {};
    this._disclosureOpen = { auto: false, engineer: false };
  }
  ...
  get isEfficiencyAuto() {
    return this.settings.inverterEfficiencyMode !== 'manual';
  }

  get isReserveRatioAuto() {
    return this.settings.reserveRatioMode !== 'manual';
  }

  get isBatteryReserveRatioAuto() {
    return this.settings.batteryReserveRatioMode !== 'manual';
  }

  renderAutoBadge() {
    return `<span class="settings-form__auto-badge">Авто</span>`;
  }
  ...
  render() {
    return `
      <section class="settings-form">
        ...
        <ui-disclosure label="Авто-параметри (можна перевизначити)" data-disclosure="auto" ${this._disclosureOpen.auto ? 'open' : ''}>
          <div class="settings-form__advanced">
            <div class="settings-form__smart-field">
              ...
              <ui-select name="batteryVoltage" label="" ${this.isVoltageAuto ? 'disabled' : ''}></ui-select>
              ${this.renderModeHint({
                isAuto: this.isVoltageAuto,
            ...
            <div class="settings-form__smart-field">
              ...
              <ui-select name="batteryType" label="" ${this.isBatteryTypeAuto ? 'disabled' : ''}></ui-select>
              ${this.renderModeHint({
                isAuto: this.isBatteryTypeAuto,
            ...
            <div class="settings-form__smart-field">
              <div class="settings-form__field-head">
                <div class="settings-form__field-label-row">
                  <span class="settings-form__field-label">ККД інвертора</span>
                  ${this.isEfficiencyAuto ? this.renderAutoBadge() : '<span class="settings-form__manual-badge">Вручну</span>'}
                </div>
                ${this.renderModeAction({
                  isAuto: this.isEfficiencyAuto,
                  toggleField: 'inverterEfficiency',
                  autoText: 'Змінити вручну',
                  manualText: 'Повернути авто',
                })}
              </div>
              <ui-input type="text" name="inverterEfficiency" label="" placeholder="0,92" value="${this.draft.inverterEfficiency}" error="${this.errors.inverterEfficiency || ''}" ${this.isEfficiencyAuto ? 'disabled' : ''}></ui-input>
              ${this.renderModeHint({
                isAuto: this.isEfficiencyAuto,
                autoHint: 'Типове значення 0,92 для побутових інверторів',
                manualHint: 'Значення задано вручну',
              })}
            </div>
          </div>
        </ui-disclosure>

        <ui-disclosure label="Інженерні параметри (запас по інвертору і АКБ)" data-disclosure="engineer" ${this._disclosureOpen.engineer ? 'open' : ''}>
          <div class="settings-form__advanced">
            <div class="settings-form__smart-field">
              <div class="settings-form__field-head">
                <div class="settings-form__field-label-row">
                  <span class="settings-form__field-label">Запас по інвертору</span>
                  ${this.isReserveRatioAuto ? this.renderAutoBadge() : '<span class="settings-form__manual-badge">Вручну</span>'}
                </div>
                ${this.renderModeAction({
                  isAuto: this.isReserveRatioAuto,
                  toggleField: 'reserveRatio',
                  autoText: 'Змінити вручну',
                  manualText: 'Повернути авто',
                })}
              </div>
              <ui-input type="text" name="reserveRatio" label="" placeholder="1,2" value="${this.draft.reserveRatio}" error="${this.errors.reserveRatio || ''}" ${this.isReserveRatioAuto ? 'disabled' : ''}></ui-input>
              ${this.renderModeHint({
                isAuto: this.isReserveRatioAuto,
                autoHint: 'Типовий коефіцієнт 1,2 (20% запасу)',
                manualHint: 'Значення задано вручну',
              })}
            </div>
            ...
            <div class="settings-form__smart-field">
              <div class="settings-form__field-head">
                <div class="settings-form__field-label-row">
                  <span class="settings-form__field-label">Запас по АКБ</span>
                  ${this.isBatteryReserveRatioAuto ? this.renderAutoBadge() : '<span class="settings-form__manual-badge">Вручну</span>'}
                </div>
                ${this.renderModeAction({
                  isAuto: this.isBatteryReserveRatioAuto,
                  toggleField: 'batteryReserveRatio',
                  autoText: 'Змінити вручну',
                  manualText: 'Повернути авто',
                })}
              </div>
              <ui-input type="text" name="batteryReserveRatio" label="" placeholder="1,15" value="${this.draft.batteryReserveRatio}" error="${this.errors.batteryReserveRatio || ''}" ${this.isBatteryReserveRatioAuto ? 'disabled' : ''}></ui-input>
              ${this.renderModeHint({
                isAuto: this.isBatteryReserveRatioAuto,
                autoHint: 'Типовий коефіцієнт 1,15 (15% запасу)',
                manualHint: 'Значення задано вручну',
              })}
            </div>
          </div>
  ...
  afterRender() {
    ...
    this.shadowRoot
      .querySelectorAll('ui-disclosure[data-disclosure]')
      .forEach((disclosure) => {
        disclosure.addEventListener('ui-disclosure-toggle', (event) => {
          const key = disclosure.getAttribute('data-disclosure');
          if (!key) return;
          this._disclosureOpen[key] = Boolean(event.detail?.open);
        });
      });

    if (this.hasAttribute('compact')) {
      this.shadowRoot.querySelectorAll('ui-input, ui-select').forEach((el) => {
  ...
  handleSelectionModeToggle = (event) => {
    ...
    if (field === 'inverterEfficiency') {
      const nextSettings = {
        ...this._settings,
        inverterEfficiencyMode: nextMode,
      };

      if (nextMode === 'auto') {
        nextSettings.inverterEfficiency = 0.92;
      }

      this._settings = nextSettings;
      this.draft = { ...this.draft, inverterEfficiency: String(nextSettings.inverterEfficiency) };
      this.emitSettingsChange();
      this.update();
      return;
    }
    ...
    if (field === 'reserveRatio') {
      const nextSettings = {
        ...this._settings,
        reserveRatioMode: nextMode,
      };

      if (nextMode === 'auto') {
        nextSettings.reserveRatio = 1.2;
      }

      this._settings = nextSettings;
      this.draft = { ...this.draft, reserveRatio: String(nextSettings.reserveRatio) };
      this.emitSettingsChange();
      this.update();
      return;
    }

    if (field === 'batteryReserveRatio') {
      const nextSettings = {
        ...this._settings,
        batteryReserveRatioMode: nextMode,
      };

      if (nextMode === 'auto') {
        nextSettings.batteryReserveRatio = 1.15;
      }

      this._settings = nextSettings;
      this.draft = { ...this.draft, batteryReserveRatio: String(nextSettings.batteryReserveRatio) };
      this.emitSettingsChange();
      this.update();
    }
  };
