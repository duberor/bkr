...
function getPrimaryBatteryConfig(calc = {}) {
  return calc?.selectedBatteryConfig || calc?.recommendedBatteryConfigs?.[0] || null;
}
...
class ReportSheet extends BaseElement {
  constructor() {
    ...
    this._activeVariantKey = (typeof sessionStorage !== 'undefined'
      ? sessionStorage.getItem('ups:report:variant')
      : null) || null;
  }
  ...
  get activeVariant() {
    const variants = this.solutionVariants;
    if (this._activeVariantKey) {
      const found = variants.find((v) => v.key === this._activeVariantKey);
      if (found) return found;
    }
    return variants.find((v) => v.isRecommended) || variants[0] || null;
  }

  get reportCalc() {
    return this.activeVariant?.calc || this.calc;
  }
  ...
  get reportConsumers() {
    return this.activeVariant?.consumers?.length
      ? this.activeVariant.consumers
      : this._state.consumers;
  }

  get bestConfig() {
    return this.calc.selectedBatteryConfig || this.calc.recommendedBatteryConfigs?.[0] || null;
  }
  ...
  get warnings() {
    ...
    const batteryReserveRatio = Number(this.reportSettings.batteryReserveRatio || 1);
    const dcCurrent = this.fullDcCurrent;
    ...
    // Структурні (важливі завжди)
    if (calc.startupCoverageRatio < 1 && calc.recommendedInverterPower > 0) {
      warnings.push(
        `Пусковий пік ${formatPower(calc.totalSurgePower)} перевищує рекомендований інвертор ${formatPower(calc.recommendedInverterPower)} — оберіть більшу модель.`,
      );
    }
    if (calc.totalPower > 1200 && voltage === 12) {
      warnings.push(
        'При 12 V DC і навантаженні понад 1.2 кВт струми зашкалюють — переходьте на 24 V або 48 V.',
      );
    }
    if (dcCurrent > 140) {
      warnings.push(
        `DC струм ${formatNumber(dcCurrent, 0)} А — потрібен мідний кабель великого перерізу і потужний автомат.`,
      );
    }

    // Варіант-залежні
    if (variant?.key === 'economy') {
      warnings.push(
        'Варіант «Мінімум» розрахований на ~70% часу. У холодну/похмуру добу запасу не вистачить.',
      );
    }
    if (variant?.key === 'essentials' && variant.deferredItems?.length) {
      warnings.push(
        `Зі сценарію прибрано необов’язкові прилади: ${variant.deferredItems.join(', ')}. Без них список покупок коректний — інакше треба перейти на «Стандарт» або «З запасом».`,
      );
    }
    if (batteryReserveRatio < 1.05 && variant?.key !== 'reliability') {
      warnings.push(
        'Запас по АКБ не закладено. З часом ємність батарей падає на 10–20% — реальний час роботи буде меншим за розрахунковий.',
      );
    }
  ...
  buildRozetkaSearchUrl(query = '') {
    return `https://rozetka.com.ua/ua/search/?text=${encodeURIComponent(String(query || '').trim())}`;
  }
  ...
  get shoppingItems() {
    ...
    const best = this.reportBestConfig;
    const batteryTypeLabel =
      batteryTypeLabels[settings.batteryType] ||
      String(settings.batteryType || 'lifepo4').toUpperCase();
    const voltage = Number(settings.batteryVoltage || 24);
    const inverterPower = Math.max(0, Math.round(calc.recommendedInverterPower || 0));
    const moduleCapacity = Math.max(0, Number(best?.moduleCapacityAh || 100));
    const moduleCount = Math.max(0, Number(best?.totalBatteries || 0));
    const bankTopology = best ? formatBatteryTopology(best.seriesCount, best.parallelCount) : '—';
    const chargeCurrent = Math.max(5, Math.round(calc.recommendedChargeCurrentA || 0));
    const fuseCurrent = Math.max(
      16,
      Math.ceil((this.surgeDcCurrent || this.fullDcCurrent || 0) * 1.25 / 5) * 5,
    );

    return [
      {
        name: 'Інвертор',
        details: `${formatPower(inverterPower)} · чиста синусоїда · ${voltage} В`,
        query: `інвертор чиста синусоїда ${inverterPower} Вт ${voltage} В`,
      },
      {
        name: 'Акумулятори',
        details: `${moduleCount || '—'} шт · ${batteryTypeLabel} · ${moduleCapacity} Ah`,
        query: `акумулятор ${batteryTypeLabel} 12V ${moduleCapacity}Ah`,
      },
      {
        name: 'Зарядний пристрій',
        details: `${voltage} В · від ${chargeCurrent} А`,
        query: `зарядний пристрій для акумулятора ${voltage}V ${chargeCurrent}A`,
      },
      {
        name: 'DC захист',
        details: `автомат/запобіжник ≈ ${fuseCurrent} А · шина ${bankTopology}`,
        query: `DC автомат ${fuseCurrent}A ${voltage}V для інвертора`,
      },
    ];
  }
  ...
  renderVariantBadges(items, type) {
    if (!items.length) {
      return type === 'on'
        ? '<span class="report-sheet__sv-allok">Усе обладнання ✓</span>'
        : '<span class="report-sheet__sv-none">—</span>';
    }
    return items
      .map((label) => `<span class="report-sheet__sv-tag report-sheet__sv-tag--${type}">${escapeHtml(label)}</span>`)
      .join('');
  }
  ...
  renderVariantCards() {
    ...
    const inverters = this.solutionVariants.map((v) => Number(v.calc.recommendedInverterPower || 0));
    const sharedInverter = inverters.length > 1 && inverters.every((p) => p === inverters[0]);

    const deferredReason = {
      essentials: 'Прибрано прилади з пріоритетом «Необов’язково»',
    };

    const activeKey = this.activeVariant?.key;

    return `
      ${sharedInverter ? `<p class="report-sheet__sv-note">Спільний інвертор у всіх варіантах: ${formatPower(inverters[0])}. Відмінність — у комплекті АКБ і наборі підключених приладів.</p>` : ''}
      <div class="report-sheet__sv-grid">
        ${this.solutionVariants
          .map((variant) => {
            const variantConfig = getPrimaryBatteryConfig(variant.calc);
            const voltage = variantConfig?.bankVoltage || variant.calc?.normalizedSettings?.batteryVoltage || 24;
            const btype = (variant.calc?.normalizedSettings?.batteryType || '').toUpperCase().replace('LIFEPO4', 'LiFePO4');
            const bankLine = variantConfig
              ? `${variantConfig.totalBatteries}× ${variantConfig.moduleVoltage}V ${variantConfig.moduleCapacityAh} Ah${btype ? ` · ${btype}` : ''}`
              : formatBatteryBank(null, variant.calc);
            const reason = deferredReason[variant.key];
            const isActive = variant.key === activeKey;
            return `
              <article class="report-sheet__sv-card ${variant.isRecommended ? 'is-rec' : ''} ${isActive ? 'is-active' : ''}">
                <div class="report-sheet__sv-top">
                  <span class="report-sheet__sv-label">${escapeHtml(variant.title)}</span>
                  ${variant.isRecommended ? '<span class="report-sheet__sv-rec">Рекомендовано</span>' : ''}
                  ${isActive && !variant.isRecommended ? '<span class="report-sheet__sv-rec">Обрано</span>' : ''}
                </div>
                ${variant.strategy ? `<div class="report-sheet__sv-strategy">${escapeHtml(variant.strategy)}</div>` : ''}
                <div class="report-sheet__sv-spec">
                  ${formatPower(variant.calc.recommendedInverterPower)} · ${formatBatteryBank(variantConfig, variant.calc)}
                </div>
                <div class="report-sheet__sv-bank">${bankLine}</div>
                <div class="report-sheet__sv-auto">~${formatAutonomy(variant.calc.estimatedAutonomyHours)} · ${formatNumber(voltage, 0)} В</div>
                <div class="report-sheet__sv-lists">
                  <div class="report-sheet__sv-row">${this.renderVariantBadges(variant.activeItems, 'on')}</div>
                  ${variant.deferredItems.length ? `
                    <div class="report-sheet__sv-row">${this.renderVariantBadges(variant.deferredItems, 'off')}</div>
                    ${reason ? `<div class="report-sheet__sv-reason">${reason}</div>` : ''}
                  ` : ''}
                </div>
              </article>
            `;
  ...
  renderVariantPicker() {
    ...
    const activeKey = this.activeVariant?.key;
    return `
      <div class="report-sheet__variant-picker" role="tablist" aria-label="Варіант для звіту">
        ${variants
          .map(
            (v) => `
              <button type="button"
                      class="report-sheet__variant-chip ${activeKey === v.key ? 'is-active' : ''}"
                      data-variant-key="${escapeHtml(v.key)}"
                      role="tab"
                      aria-selected="${activeKey === v.key ? 'true' : 'false'}">
                ${escapeHtml(v.title)}
              </button>`,
          )
          .join('')}
  ...
  renderShoppingRows() {
    ...
    if (!items.length) {
      return '<p class="report-sheet__text-muted">Додайте прилади, щоб сформувати список покупок.</p>';
    }

    return `
      ${this.renderVariantPicker()}
      <div class="report-sheet__shopping-list">
        ${items
          .map(
            (item) => `
          <div class="report-sheet__shopping-item">
            <div class="report-sheet__shopping-main">
              <strong>${escapeHtml(item.name)}</strong>
              <span>${escapeHtml(item.details)}</span>
            </div>
            <div class="report-sheet__shopping-links">
              <a href="${this.buildGoogleSearchUrl(item.query)}" target="_blank" rel="noopener noreferrer">Google</a>
              <a href="${this.buildRozetkaSearchUrl(item.query)}" target="_blank" rel="noopener noreferrer">Rozetka</a>
            </div>
          </div>
        `,
          )
  ...
  render() {
    ...
    const isCustomVariant = activeVariant && recommendedVariant && activeVariant.key !== recommendedVariant.key;
    const voltage = Number(this.reportSettings.batteryVoltage || 24);
    ...
    return `
      <ui-card padding="lg">
        <article class="report-sheet">
            ...
            <p class="report-sheet__summary-text">
              ${
                this._state.consumers.length
                  ? `${isCustomVariant ? `Звіт сформовано під обраний варіант «${activeVariant.title}» (рекомендований — «${recommendedVariant.title}»).` : `Для цього сценарію найкраще підходить варіант «${activeVariant?.title || 'Стандарт'}».`} Він розрахований на ${formatPower(calc.designLoadPower)} робочого навантаження, враховує пусковий пік ${formatPower(calc.totalSurgePower)} і дає приблизно ${formatAutonomy(calc.estimatedAutonomyHours)} роботи у звичному режимі. Комплект: інвертор ${formatPower(calc.recommendedInverterPower)} і батарейний банк ${recommendedBankLabel}${best ? ` (${recommendedTopology})` : ''}.`
          ...
          <section class="report-sheet__section report-sheet__section--shopping" data-anchor="shopping">
            <div class="report-sheet__title-row">
              <h3>Список покупок</h3>
              <span class="report-sheet__tag report-sheet__tag--key">Ключове</span>
            </div>
            ${this.renderShoppingRows()}
          </section>
  ...
  afterRender() {
    ...
    this.shadowRoot.querySelectorAll('[data-variant-key]').forEach((button) => {
      button.addEventListener('click', this.handleVariantSwitch);
    });

    const target = sessionStorage.getItem('ups:report:scroll-target');
    if (target !== 'shopping') return;

    sessionStorage.removeItem('ups:report:scroll-target');
    const anchor = this.shadowRoot.querySelector('[data-anchor="shopping"]');
    if (!anchor) return;
  ...
  handleVariantSwitch = (event) => {
    const key = event.currentTarget?.dataset?.variantKey;
    if (!key || key === this._activeVariantKey) return;
    this._activeVariantKey = key;
    try {
      sessionStorage.setItem('ups:report:variant', key);
    } catch (_) { /* ignore */ }
    if (this.isConnected) this.update();
  };
}
